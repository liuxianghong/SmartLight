...
+(LXURLSessionTask *)baseRequestType:(NSUInteger)type
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@%@",HTTPBASEURL,urlStr];
    DLog(@"请求地址----%@\n    请求参数----%@",urlString,params);
    
    if (type==1) {
       sessionTask = [manager GET:urlString parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
       ...
       sessionTask = [manager POST:urlString parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
...
+(LXURLSessionTask *)uploadWithImage:(UIImage *)image
    ...
    LXURLSessionTask *sessionTask = [manager POST:urlString parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //压缩图片
