//
//  GroupSetUpTimeViewController.m
//  Intelligentlamp
//
...
@interface GroupSetUpTimeViewController()<UIPickerViewDelegate>
...
@implementation GroupSetUpTimeViewController
...
- (void)viewDidLoad{
    ...
    [self leftBarButtonItem:[UIImage imageNamed:@"BackArrow"] withClickBtnAction:^{
        ...
        if (self.scenarioSetUpBlock) {
            self.scenarioSetUpBlock(_model);
        }
    }];
    
    //设置初始的默认开关灯时间
    
    if (_model.ScenarioOpenTime == nil ||[_model.ScenarioOpenTime isEqual:[NSNull null]] || [_model.ScenarioOpenTime isEqualToString:@""]) {
        [_leftTimeBtn setTitle:@"开灯时间 00:00:00" forState:UIControlStateNormal];
    }else {
        
        
         [_leftTimeBtn setTitle:[NSString stringWithFormat:@"开灯时间 %@",_model.ScenarioOpenTime] forState:UIControlStateNormal];
    }
    
    if ([_model.ScenarioCloseTime isEqual:[NSNull null]]||[_model.ScenarioCloseTime isEqualToString:@""]||_model.ScenarioCloseTime == nil) {
        
        [_rightTimeBtn setTitle:@"关灯时间 00:00:00" forState:UIControlStateNormal];
    }else {
        [_rightTimeBtn setTitle:[NSString stringWithFormat:@"关灯时间 %@",_model.ScenarioCloseTime ] forState:UIControlStateNormal];
    }
    
    //设置默认的定时开关按钮状态
    
    if (_model.ScenarioTimingOn == 1) {
        _switchBtn.on = YES;
    }else{
        _switchBtn.on = NO;
    }
    
    if (_model.ScenarioRandomOn == 1) {
...
- (void)deterBtnClick{
    ...
    if (_isLeft) {
        if ([_time isEqualToString:@""]||[_time isEqual:[NSNull class]]) {
            ...
            _model.ScenarioOpenTime = dateString;
        }else{
        ...
        if ([_time isEqualToString:@""]) {
            ...
            _model.ScenarioCloseTime = dateString;
        }else{
...
- (void)switchBtnClick:(id)sender{
    ...
    if (isButtonOn) {
        NSLog(@"时间选择器打开");
        _model.ScenarioTimingOn = 1;
    }else {
        NSLog(@"时间选择器关闭");
        _model.ScenarioTimingOn = 0;
    }
...
- (void)randomSwitchBtnClick:(id)sender{
    ...
    if (isButtonOn) {
        NSLog(@"随机时间选择器打开");
        _model.ScenarioRandomOn = 1;
    }else {
        NSLog(@"随机时间选择器关闭");
        _model.ScenarioRandomOn = 0;
    }
