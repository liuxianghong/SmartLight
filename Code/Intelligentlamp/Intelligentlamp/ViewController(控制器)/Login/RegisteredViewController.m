...
#define PhoneTitle          @"请输入手机号码"
#define PwdTitle            @"设置密码"
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setTitleViewWithStr:NSLocalizedString(@"Register", nil)];
    [self layoutWithUI];
...
//    bgPwd.layer.cornerRadius                         = 2;
    ...
    _pwdTxtField.attributedPlaceholder               = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"SetPassword", nil) attributes:@{NSForegroundColorAttributeName:AllLampTitleColor}];
...
//    _getCodeBtn.layer.cornerRadius                     = 2;
    ...
    [_getCodeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(_pwdBgImgView.mas_bottom).offset(15);
        make.left.mas_equalTo(_codeBgImgView.mas_right).offset(15);
        make.width.mas_equalTo((ScreenWidth - 60)/2);
    ...
    [_RegisteredBtn setTitle:NSLocalizedString(@"Register", nil) forState:UIControlStateNormal];
